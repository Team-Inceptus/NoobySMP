	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		try {
			...
			switch (args[2].toLowerCase()) {
				case "level": {
					sender.sendMessage(ChatColor.GOLD + p.getName() + "'s " + ChatColor.GREEN + "Level is " + ChatColor.GOLD + Integer.toString(config.getLevel(type)));
					return true;
				}
				case "exp": {
					sender.sendMessage(ChatColor.GOLD + p.getName() + "'s " + ChatColor.GREEN + "Base Experience Amount is " + ChatColor.GOLD + Double.toString(config.getExperience()));
					return true;
				}
				case "setlevel": {
					if (args.length < 4) {
