...
public class ServerManager implements Listener {
	...
	public static String parseDeathMessage(DamageCause c, Entity damager) {
		...
		switch (c) {
			case ENTITY_SWEEP_ATTACK, ENTITY_ATTACK -> {
	...
	public static String parseDeathMessage(DamageCause c) {
		switch (c) {
	...
	private static String formatList(Player p, String msg, boolean quit) {
	...
	private static String formatList(Player p, String msg) {
		return formatList(p, msg, false);
	}
	
	private static void setResourcePack(Player p) {
			((CraftPlayer) p).getHandle().sendTexturePack(PACK_URL, "6FF5086013892BAA1160128BA9295A2615D551D4", false, Component.literal("NoobySMP has a resource pack that may help items render better. This is not required to accept. It will not override (most) existing textures."));
	}
