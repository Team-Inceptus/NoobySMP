...
public class Tags implements TabExecutor {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) suggestions.addAll(Arrays.asList("add", "list"));
            ...
            List<AbilityItem> items = Arrays.stream(AbilityItem.values()).filter(a -> SMPTag.getByOrigin(a) != null).toList();
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "list" -> {
                ...
                for (SMPTag<?> tag : tags)
                    tagInfo.add(ChatColor.GOLD + "Name: " + ChatColor.AQUA + tag.getName() +
