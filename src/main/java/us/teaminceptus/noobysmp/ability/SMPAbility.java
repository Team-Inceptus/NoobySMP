...
public enum SMPAbility {
	...
	FIRE_WAND("Fire Shoot", RIGHT_CLICK(), 20, AbilityItem.FIRE_WAND, p -> {
		...
		for (double i = 0.5; i < 5; i += 0.5) {
			...
			p.getWorld().getNearbyEntities(target, 1, 1, 1).forEach(e -> e.setFireTicks(20 * (r().nextInt(10) + 2 + (int) Math.floor((double) new PlayerConfig(p).getLevel() / 3))));
		}
	}),
...
//	}
	...
	private static Action[] RIGHT_CLICK() { return new Action[] {Action.RIGHT_CLICK_AIR, Action.RIGHT_CLICK_BLOCK}; }
	private static Action[] LEFT_CLICK() { return new Action[] {Action.LEFT_CLICK_AIR, Action.LEFT_CLICK_BLOCK }; }
	...
	SMPAbility(String name, Action[] action, long cooldown, AbilityItem item, Consumer<Player> output) {
		this.name = name;
		this.actions = action;
		this.item = item;
		this.output = output;
	...
	SMPAbility(String name, Action[] action, long cooldown, AbilityItem item, BiConsumer<Player, LivingEntity> output) {
		this.name = name;
		this.actions = action;
		this.item = item;
		this.output = null;
	...
	public static SMPAbility getByItem(AbilityItem item) {
